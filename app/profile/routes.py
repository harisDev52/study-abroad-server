    ...
    try:
        ...
        with open(file_path, mode='r', encoding='utf-8') as csv_file:
            ...
            for row in csv_reader:
                domain = row['Domain']
                uni_name = row['Uni Name']
                description = row['Description']
                descriptions.setdefault(domain, {}).update({uni_name: description})
        return descriptions
    except FileNotFoundError:
        raise FileNotFoundError(f"File '{file_path}' not found.")
    except Exception as e:
...
def get_description_by_domain(descriptions, domain, uni_name):
    if domain in descriptions:
        uni_descriptions = descriptions[domain]
        if uni_name in uni_descriptions:
            return uni_descriptions[uni_name]
...
def get_reviews_by_university(reviews, university):
    ...
    for item in all_reviews:
        if item.get('Uni Name') == university:
...
def init_profile_routes(app):
    ...
    @app.route('/description/<domain>/<university>', methods=['GET'])
    def get_university_description(domain, university):
        description = get_description_by_domain(descriptions, domain, university)
        if description != 'Description not available':
            return jsonify({'description': description}), 200
        else:
            return jsonify({'message': f'Description not found for "{university}" in the domain "{domain}"'}), 404
